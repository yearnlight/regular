<template>
    <div class="regular">
        ...
        <div class="regular-create">
            <el-button plain @click="create">添加正则</el-button>
            <el-input
        ...
        <el-card v-else v-for="(item,index) in list" :key="index" shadow="hover">
            ...
            <div class="regular-exam">
                ...
                ></el-input>
                <span
                    :class="[{'green':item.result&&item.result.includes('通过'),'red':item.result&&item.result.includes('失败')},'result']"
                >{{item.result}}</span>
            </div>
        ...
        <el-dialog title="创建正则" custom-class="regularCreate" :visible.sync="isCreate">
            ...
            <el-form :rules="rules" ref="createRegular" :model="form" label-width="140px">
                ...
                <el-form-item label="测试数据" prop="test" required>
                    ...
                    ></el-input>
                    <span
                        :class="[{'green':form.result&&form.result.includes('通过'),'red':form.result&&form.result.includes('失败')},'result']"
                    >{{form.result}}</span>
                </el-form-item>
                <el-form-item label="创建者" prop="creater" required>
                    <el-input
                        size="small"
                        maxlength="30"
                        placeholder="请填写创建者"
  ...
  data() {
    return {
      ...
      rules: {
        ...
        creater: [{ required: true, message: "创建者必填" }],
  ...
  methods: {
    ...
    testExec(v, item) {
      if(/^\/.*\/$/.test(item.expression)){
        item.expression = item.expression.substring(1,item.expression.length - 1);
      }
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          if(/^\/.*\/$/.test(this.form.expression)){
            this.form.expression = this.form.expression.substring(1,this.form.expression.length - 1);
          }
